...
read dades
pid=`echo $dades | awk -F "&" '{print $1}' | awk -F "=" '{print $2}'`
sleep=`echo $dades | awk -F "&" '{print $2}' | awk -F "=" '{print $2}'`

echo "</br><h2><div>
	Sleeping process $sleep s with pid = $pid ...
	</div></h2>"
...
sh go_to_sleep.sh $pid $sleep >/dev/null &
